...
![](http://cranlogs.r-pkg.org/badges/SMFilter?color=green) 
![](http://cranlogs.r-pkg.org/badges/last-week/SMFilter?color=green) 
...
ggplot() + geom_point(aes(x=1:iT,y=sapply(1:iT,ftmp)/4/ir)) +
  ylim(0, 1) + labs(x=paste0("t= 1, ...,",iT), y="normalized \u03b4( \u03b1, U )")
```

What if we start with a wrongly specified initial value, $-\boldsymbol{\alpha}_0$ cannot be worse?
...
# plot the distances
ggplot() + geom_point(aes(x=1:iT,y=sapply(1:iT,ftmp)/4/ir)) +
  ylim(0, 1) + labs(x=paste0("t= 1, ...,",iT), y="normalized \u03b4( \u03b2, U )")
```

What if we start with a wrongly specified initial value, $-\boldsymbol{\beta}_0$ cannot be worse?
